...
func (yc *Cloud) InstanceID(ctx context.Context, nodeName types.NodeName) (string, error) {
	...
	return generateInstanceID(instance.ZoneId, instance.Id), nil
}
...
func (yc *Cloud) getInstanceByProviderID(ctx context.Context, providerID string) (*compute.Instance, error) {
	_, instanceName, instanceNameIsId, err := ParseProviderID(providerID)
	if err != nil {
		return nil, err
	}

	if instanceNameIsId {
		return yc.yandexService.ComputeSvc.InstanceSvc.Get(ctx, &compute.GetInstanceRequest{InstanceId: instanceName})
	}

	return yc.yandexService.ComputeSvc.FindInstanceByName(ctx, instanceName)
}
