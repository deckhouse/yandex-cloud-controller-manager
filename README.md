...
## Нюансы
* `1. Если создаются мастера со своими таргет группами, то нужно навешивать на них лейбл node.kubernetes.io/exclude-from-external-load-balancers: "", что бы контроллер не пытался добавить мастера вновую таргет группу для балансеров `
* `2. Если хотите сами добавлять балансер для мастеров да так, что бы еще и ycc мог на мастера навешивать свои балансеры, создайте заранее таргет группу по маске имени ${CLUSTER-NAME}${VPC.ID}`
